...
class RandomImagePixelationDataset(Dataset):
    ...
    def load_images(self):
        # Implement a function to load and preprocess the images
        return sorted(glob.glob(os.path.join(self.image_dir, "**", "*.jpg"), recursive=True))
    ...
    def __getitem__(self, idx):
        ...
        with Image.open(self.images[idx]) as im:  # This returns a PIL image
        ...
        print(height)
        x = int(rng.uniform(low=0, high = self.width_range[1] - width))
        print(x)
        y = int(rng.uniform(low=0, high = self.width_range[1] - width))
        print(y)
        size = int(rng.random() * (self.size_range[1] - self.size_range[0]) + self.size_range[0])
        print(size)
        image = im_proc_f.prepare_image(image, x, y, width, height, size)
        return image
